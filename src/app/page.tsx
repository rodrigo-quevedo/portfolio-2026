...
import { FaHtml5, FaCss3Alt, FaReact, FaNodeJs, FaJava, FaDocker, FaGitAlt, FaGithub, FaNetworkWired, FaProjectDiagram, FaCode, FaLinkedin, FaDatabase } from "react-icons/fa";
...
// Mapeo de iconos basado en el nombre de la tecnología
const getSkillIcon = (skill: string, className: string = "w-4 h-4") => {
  const iconMap: Record<string, { icon: React.ElementType, color: string }> = {
    ...
    "Express.js": { icon: SiExpress, color: "#ffffff" }, // Usado en proyectos
    ...
    "Prisma": { icon: SiPrisma, color: "#2D3748" }, // Color de Prisma
    "Supabase": { icon: SiSupabase, color: "#3ECF8E" }, // Color de Supabase
  ...
  // Si no se encuentra el icono, retorna un icono de código genérico
  return <FaCode className={className} style={{ color: "#94a3b8" }} />; // color slate-400 por defecto
};
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 items-center gap-12">
          ...
          >
            ...
            <div className="flex flex-wrap justify-center lg:justify-start gap-6 text-sm font-medium text-slate-400 mb-10">
              ...
              <span className="flex items-center gap-2"><IoMdMail  className="w-5 h-5" /> {info.personal.emails[0]}</span>
            </div>
            <div className="flex justify-center lg:justify-start gap-4">
              ...
              <a href={info.personal.linkedin} target="_blank" rel="noreferrer" className="flex items-center gap-2 px-8 py-4 bg-[#0A66C2] border border-white/[0.1] text-white font-bold rounded-full backdrop-blur-md hover:bg-white/[0.1] hover:border-white/[0.2] transition-all">
        ...
        <motion.div variants={staggerContainer} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-50px" }} className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <motion.div variants={cardVariant} className={`md:col-span-2 ${glassCard}`}>
            ...
            <div className="flex flex-wrap gap-3">
                ...
                <span key={skill} className="flex items-center gap-2 px-4 py-2 bg-white/5 border border-white/10 text-slate-300 rounded-full text-sm font-medium hover:bg-white/10 transition-colors">
                  {getSkillIcon(skill)}
                  {skill}
                </span>
                ))}
              ...
              <div className={`${glassCard} relative overflow-hidden`}>
                ...
                {exp.stack && exp.stack.length > 0 && (
                  <div className="flex flex-wrap gap-2 mt-6 pt-6 border-t border-white/10">
                    {exp.stack.map(tech => (
                      <span key={tech} className="flex items-center gap-1.5 px-3 py-1 bg-white/5 border border-white/10 text-slate-300 text-xs font-medium rounded-full">
